...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        camera_pose = get_tensor_from_camera(view.world_view_transform.transpose(0, 1))#.transpose(0, 1)
        
        if view.image_name.startswith("w") and not view.image_name.startswith("u"):
            rendering = render(
                view, gaussians, pipeline, background, info={"c":1., "target":"cx"}, camera_pose=camera_pose
            )["render"]
        elif view.image_name.startswith("u") and not view.image_name.startswith("w"):
            rendering = render(
                view, gaussians, pipeline, background, info=None, camera_pose=camera_pose
            )["render"]
        else:
            print(">>> Error: view.image_name: ", view.image_name)
...
def render_set_optimize(model_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        with tqdm(total=num_iter, desc=f"Tracking Time Step: {idx+1}", leave=True) as progress_bar:
            ...
            for iteration in range(num_iter):
                rendering = render(view, gaussians, pipeline, background, info={"c":1., "target":"cx"}, camera_pose=torch.cat([camera_tensor_q, camera_tensor_T]))["render"]
        ...
        rendering_opt = render(view, gaussians, pipeline, background, info={"c":1., "target":"cx"}, camera_pose=optimal_pose)["render"]
    ...
    if args.test_fps:
        ...
        for _ in range(1000):
            start = perf_counter()
            _ = render(view, gaussians, pipeline, background, info={"c":1., "target":"cx"}, camera_pose=optimal_pose)
            end = perf_counter()
...
def render_sets(
    ...
    if not skip_train and not args.infer_video and not dataset.eval:        
        optimized_pose = load_pose(Path(args.model_path) / 'pose' / f'ours_{iteration}' / 'pose_optimized.npz')
        # R有tranpose
        
        viewpoint_stack = loadCameras(optimized_pose, scene.getTrainCameras())
        #breakpoint()
        #感覺是這邊有問題，因為這邊的viewpoint_stack是用optimized_pose來load的。可能有問題
        render_set(
            dataset.model_path,
            "train",
            scene.loaded_iter,
