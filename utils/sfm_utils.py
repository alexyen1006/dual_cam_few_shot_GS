...
def split_train_test(image_files, llffhold=8, n_views=None, verbose=True):
    ...
    train_images_name = ['uw_001', 'uw_008', 'uw_015',
                         'w_001', 'w_008', 'w_015']
    train_img_files = []
    test_img_files = []
    for image_file in image_files:
        ...
        if image_name in train_images_name:
            train_img_files.append(image_file)
        elif image_name not in train_images_name and image_name.startswith('w_'):
            test_img_files.append(image_file)
    if verbose:
        print(">> Spliting Train-Test Set: ")
        # print(" - sparse_idx:         ", sparse_idx)
        print(" - train_set_indices:  ", train_img_files)
        print(" - test_set_indices:   ", test_img_files)
...
# def split_train_test_eval(image_files, llffhold=8, n_views=None, verbose=True):
#     train_images_name = ['w_001.jpg', 'w_008.jpg', 'w_015.jpg', 'uw_001.jpg', 'uw_008.jpg', 'uw_015.jpg']
#     train_img_files = []
#     test_img_files = []
#     for image_name, pose in image_files.item():
        
#         if image_name in train_images_name:
#             train_img_files.append(image_file)
#         elif image_file.image_name not in train_images_name and image_file.image_name.startswith('w_'):
#             test_img_files.append(image_file)
#     if verbose:
#         print(">> Spliting Train-Test Set: ")
#         # print(" - sparse_idx:         ", sparse_idx)
...
def load_images(folder_or_list, size, square_ok=False, verbose=True):
    ...
    for path in folder_content:
        ...
        imgs.append(dict(img=ImgNorm(img)[None], true_shape=np.int32(
            [img.size[::-1]]), idx=len(imgs), instance=str(len(imgs)), image_name=f"{Image_Name}"))
...
# def save_extrinsic_rel(sparse_path, extrinsics_w2c):
#     breakpoint()
#     images_bin_file = sparse_path / 'images.bin'
#     images_txt_file = sparse_path / 'images.txt'
#     images = {}
    
#     w2c = extrinsics_w2c[0]
#     name = Path(img_file).stem + image_suffix
#     rotation_matrix = w2c[:3, :3]
#     qvec = rotmat2qvec(rotation_matrix)
#     tvec = w2c[:3, 3]
...
def save_intrinsics(sparse_path, focals, org_imgs_shape, imgs_shape, save_focals=False, with_GT_focal = False):
    ...
    for index, camera in cameras.items(): 
        print('camera ' ,index, 'focal:', camera.params[0])
...
def read_colmap_gt_pose_ori(gt_pose_path, llffhold=8):
...
def read_colmap_gt_pose(gt_pose_path, llffhold=8):
    ...
    for idx, key in enumerate(colmap_cam_extrinsics):
        ...
        pose[:3, 3] = T
        #breakpoint()
        all_pose[extr.name] = pose
    #colmap_pose = np.array(all_pose)
    #breakpoint()
    return all_pose

def readImages_ori(renders_dir, gt_dir):
    renders = []
    gts = []
    image_names = []
...
def readImages(renders_dir, gt_dir):
    ...
    gts = []
    image_names = []
    exclude_pics = ['w_016.png','w_017.png','w_018.png','w_019.png','w_020.png','w_021.png']
    for fname in os.listdir(gt_dir):
        if fname not in exclude_pics:
            render = PIL.Image.open(renders_dir / fname)
            gt = PIL.Image.open(gt_dir / fname)
            renders.append(tf.to_tensor(render).unsqueeze(0)[:, :3, :, :].cuda())
            gts.append(tf.to_tensor(gt).unsqueeze(0)[:, :3, :, :].cuda())
