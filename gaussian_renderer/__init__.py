...
def render(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0,
           override_color = None, white_bg = False, info=None, camera_pose=None,):
    """
    Render the scene.
    ...
    #delta_x = pc.get_delta_x
    #delta_c = pc.get_delta_c.transpose(1,2)
    delta_x = pc.get_delta_x
    delta_c = pc.get_delta_c.transpose(1,2)
    delta_s = pc.get_delta_s
    delta_r = pc.get_delta_r
    delta_o = pc.get_delta_o
    ...
    if info is not None:
        ...
        #deta_x, deta_r, deta_s, _, deta_o = pc._mlp(shs, rotations, scales, means3D, opacity, c=info['c'], target=info['target'])
        # deta_x, deta_r, deta_s, deta_c, deta_o = pc._mlp(shs, rotations, scales, means3D, opacity, c=info['c'], target=info['target']) 
        means3D = means3D + delta_x
        scales = scales + delta_s
        rotations = rotations + delta_r
        shs = shs + delta_c
