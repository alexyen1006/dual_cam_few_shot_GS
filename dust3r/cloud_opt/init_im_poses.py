...
from dust3r.cloud_opt.commons import edge_str, edge_to_ij, i_j_ij, compute_edge_scores, normalize_edges_scores
from utils.pose_utils import compute_relative_world_to_camera, compute_relative_world_to_camera_mod, quad2rotation, get_camera_from_tensor
...
@torch.no_grad()
def init_minimum_spanning_tree(self, focal_avg=False, known_focal_uw=None, known_focal_wide=None, view_mapping=None, output_path=None, **kw):
    ...
    pts3d, _, im_focals, im_poses = minimum_spanning_tree(self.imshapes, self.edges,
                                                          ...
                                                          device, has_im_poses=self.has_im_poses, verbose=self.verbose, known_focal_uw = known_focal_uw, known_focal_wide=known_focal_wide,
                                                          view_mapping=view_mapping, **kw)

    if known_focal_uw is not None and known_focal_wide is not None:
        self.preset_focal_uw(known_focal_uw=known_focal_uw)
        self.preset_focal_wide(known_focal_wide=known_focal_wide)
    elif focal_avg:
    ...
    return init_from_pts3d(self, pts3d, im_focals, im_poses, output_path)
...
def init_relative_pose(self):
    #找對應的人
    #W_UW_view_mapping[(index, wide_index)] = {image_name, wide_img_name}
    E_rel_all = []
    rotations = []
    translations = []
    for (uw_idx,wide_idx) in self.UW_W_pair:
        E_rel = self.compute_pairwise_relative_pose(uw_idx, wide_idx)
        E_rel_all.append(E_rel)
        #breakpoint()
        R_rel = E_rel[:3, :3]
        T_rel = E_rel[:3, 3]
        Q_rel = roma.rotmat_to_unitquat(R_rel)
        SO3_rel = quat_to_SO3(Q_rel.cpu().numpy())
        rotations.append(SO3_rel)
        translations.append(T_rel.cpu().numpy())
    ...
    Q = avg_rotation_quat_mod  # 确保这是正确格式的四元数
    T = torch.from_numpy(avg_translation_npy).float().cuda().unsqueeze(0)  # 转换平移向量为tensor

    avg_QT = roma.RigidUnitQuat(Q, T).normalize().to_homogeneous().squeeze(0)
    
    #avg_rotation_R_test = quad2rotation(avg_rotation_quat_mod).squeeze(0)
    avg_rotation_R = avg_QT[:3, :3]
    avg_rotation_R_npy = avg_rotation_R.cpu().numpy()
    # 平均平移向量
    
    # breakpoint()
    avg_rel_pose = np.block([[avg_rotation_R_npy, avg_translation_npy.reshape(3, 1)], [np.zeros((1, 3)), 1]])
    
    avg_rel_pose_tensor = torch.tensor(avg_rel_pose, dtype=torch.float32).cuda()
    self.relative_pose.data[:4] = torch.from_numpy(avg_rotation_quat).float().to(self.device)
    self.relative_pose.data[4:7] = torch.from_numpy(avg_translation_npy).float().to(self.device)
    
    
    
    return avg_rel_pose_tensor, E_rel_all
    
def init_from_pts3d(self, pts3d, im_focals, im_poses, output_path):
    # init poses
    #breakpoint()
    nkp, known_poses_msk, known_poses = get_known_poses(self)
    if nkp == 1:
    ...
    rel_pose_tensor, E_rel_all = init_relative_pose(self)
    initialize_im_poses(self)
    if self.verbose:
        iteration = 0

        print(' init loss =', float(self(iteration, output_path )))


def minimum_spanning_tree(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, min_conf_thr,
                          device, has_im_poses=True, niter_PnP=10, verbose=True, known_focal_uw=None, known_focal_wide=None, view_mapping=None):
    n_imgs = len(imshapes)
    ...
    if not known_focal_uw is None and not known_focal_wide is None:
        for idx, name_set in view_mapping.items():
            name = list(name_set)[0]
            if name.startswith('u') and not name.startswith('w'):
                im_focals[idx] = known_focal_uw
            elif not name.startswith('u') and name.startswith('w'):
                im_focals[idx] = known_focal_wide
    #breakpoint()
    ...
    i_j = edge_str(i, j)
    pts3d[i] = pred_i[i_j].clone() #camera i的點雲 座標在i
    pts3d[j] = pred_j[i_j].clone() #camera i的點雲 座標在j
    done = {i, j}
    if has_im_poses:
...
def get_known_poses(self):
    if self.has_im_poses:
        known_poses_msk = torch.tensor([False for i in range(len(self.im_poses))])
        for i in range(len(self.im_poses)):
            if i in self.nodes_uw:
                known_poses_msk[i] = not (self.im_poses[i].requires_grad)
            elif i in self.nodes_wide:
                known_poses_msk[i] = False
        ...
        if i in self.nodes_wide:
            for (uw_idx, wide_idx),_ in self.UW_W_pair.items():
                if i == wide_idx:
                    print("===================================")
                    #breakpoint()
                    RT_matrix = self.get_corresponding_wide_pose(uw_idx)
                    R = RT_matrix[:3, :3]
                    T = RT_matrix[:3, 3]
                    QT_wide = self._set_one_pose(R, T)
                    QT_wide_tensor = torch.tensor(QT_wide, dtype=torch.float32).cuda()
                    self.im_poses[i] = QT_wide_tensor
                    #self.im_poses[i].requires_grad = False 
                    print("------------------------------------")
