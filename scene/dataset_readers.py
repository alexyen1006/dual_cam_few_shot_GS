...
def loadCameras(poses, viewpoint_stack):
    ...
    if poses.shape[0] == len(viewpoint_stack):
        for idx, cam in enumerate(viewpoint_stack):
            #breakpoint()
            if poses[idx].shape[0] == 7:
                quat = poses[idx][:4]  # 前 4 個是四元數
                R = quad2rotation(quat.unsqueeze(0)).squeeze(0)
                R = R.cpu().numpy()
                R = np.transpose(R)
                #breakpoint()
                T = poses[idx][4:].cpu().numpy()
            elif poses[idx].shape[0] == 3 and poses[idx].shape[1] == 3:
                breakpoint()
                R = np.transpose(poses[idx])
                T = np.zeros(3)
            else:
                print("poses shape error================================================")
            # R = np.transpose(poses[idx][:3, :3])
            # T = poses[idx][:3, 3]
            #breakpoint()
            cam.R = R
            cam.T = T
            cam.world_view_transform = torch.tensor(getWorld2View2(R, T)).transpose(0, 1).cuda()#W2C
...
def readColmapSceneInfo(path, images, eval, args, llffhold=8, get_all_cam=False):
    #breakpoint()
    if eval:
    ...
    #breakpoint()
    cam_infos_unsorted, poses = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), get_all_cam="all")
    sorting_indices = sorted(range(len(cam_infos_unsorted)), key=lambda x: cam_infos_unsorted[x].image_name)
    cam_infos = [cam_infos_unsorted[i] for i in sorting_indices]
    sorted_poses = [poses[i] for i in sorting_indices]
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
    # eval_img_name_list =['w_002.jpg','w_003.jpg','w_']
